      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <p className="mt-1 text-2xl font-bold font-mono text-foreground">{analyses.filter(a => a.result).length}</p>
          </div>
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <tbody>
              {analyses.map(({ company: c, result }) => (
                <tr key={c.ticker} className="border-b border-border/50 hover:bg-accent/30 transition-colors">
                  <td className="px-3 py-2.5">{result ? <StatusBadge status={result.status} /> : <span className="text-xs text-muted-foreground">—</span>}</td>
                  ...
                  <td className="px-3 py-2.5 font-mono text-xs">{result ? formatCurrency(result.intrinsicValuePerShare) : "—"}</td>
                  <td className="px-3 py-2.5 font-mono text-xs">{result ? formatCurrency(result.intrinsicWithMargin) : "—"}</td>
                  <td className="px-3 py-2.5">
                    {result ? (
